...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options['skip_fetch']:
            ...
            self.stdout.write("⏭️  STEP 1/4: Skipping stock data fetch")
            steps_completed += 1
        ...
        if not options['skip_export']:
            ...
            self.stdout.write("⏭️  STEP 2/4: Skipping data export")
            steps_completed += 1
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f"⚠️ Data quality check failed: {e}"))
            steps_completed += 1
        ...
        if not options['skip_notifications']:
            ...
            self.stdout.write("⏭️  STEP 4/4: Skipping email notifications")
            steps_completed += 1
        ...
        self.stdout.write("=" * 60)
        self.stdout.write(f"⏱️  Total time: {int(workflow_time//60)} min {int(workflow_time%60)} sec")
        ...
        if steps_completed == total_steps:
            ...
            self.stdout.write(self.style.WARNING(f"⚠️ Workflow partially completed ({steps_completed}/{total_steps} steps)"))
