...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🏛️  Loading NASDAQ-Only Ticker List')
        )
    ...
    def show_nasdaq_summary(self, tickers: list, ticker_file: Path):
        ...
        self.stdout.write(f'   🏛️  Exchange: NASDAQ ONLY')
    ...
    def dry_run_analysis(self, tickers_to_load: list, update_existing: bool):
        ...
        self.stdout.write(f'   ⏭️  NASDAQ tickers to skip: {len(existing_nasdaq & set(tickers_to_load)) - len(existing_to_update):,}')
    ...
    def load_nasdaq_tickers(self, tickers_to_load: list, options: dict):
                    ...
                    for ticker in batch_tickers:
                        try:
                            ...
                            if error_count <= 5:  # Show first 5 errors
                                self.stdout.write(
                                    self.style.WARNING(f'   ⚠️  Error processing NASDAQ ticker {ticker}: {e}')
                                )
                            continue
        ...
        self.stdout.write(f'   ⏭️  Skipped: {skipped_count:,} unchanged NASDAQ tickers')
        if error_count > 0:
        ...
        self.stdout.write(f'   🏛️  NASDAQ stocks: {nasdaq_stocks:,}')
