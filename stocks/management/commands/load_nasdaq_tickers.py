...
self.stdout.write(f' ⏭ Existing tickers to skip: {len(existing_tickers & set(tickers_to_load)) - len(existing_to_update):,}')
...
error_count += 1
self.stdout.write(
self.style.WARNING(f' Error processing {ticker}: {e}')
)
continue
...
self.stdout.write(f' ⏭ Skipped: {skipped_count:,} unchanged tickers')
if error_count > 0:
