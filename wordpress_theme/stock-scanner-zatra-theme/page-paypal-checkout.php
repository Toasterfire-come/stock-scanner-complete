...
// Validate plan (plugin currently supports bronze/silver/gold)
$valid_plans = ['bronze', 'silver', 'gold'];
if (!in_array($selected_plan, $valid_plans)) {
    // Map theme plans to plugin plans
    $map = ['pro' => 'silver', 'enterprise' => 'gold', 'free' => 'bronze'];
    $selected_plan = $map[$selected_plan] ?? 'bronze';
}
...
<style>
.paypal-checkout-container { max-width: 1200px; margin: 0 auto; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
.checkout-header { text-align: center; margin-bottom: 40px; }
.checkout-header h1 { font-size: 2.5rem; color: #2c3e50; margin: 0 0 10px 0; }
.checkout-header p { color: #7f8c8d; font-size: 1.1rem; }
.checkout-content { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 40px; }
.plan-summary, .payment-section { background: white; padding: 30px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border: 1px solid #e1e5e9; }
.plan-summary h2, .payment-section h2 { margin: 0 0 25px 0; color: #2c3e50; font-size: 1.5rem; }
.selected-plan { background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 25px; border-left: 4px solid #3498db; }
.plan-badge { font-size: 1.2rem; font-weight: bold; margin-bottom: 15px; }
.plan-badge.bronze { color: #cd7f32; }
.plan-badge.silver { color: #c0c0c0; }
.plan-badge.gold { color: #ffd700; }
.plan-details { display: flex; align-items: center; justify-content: space-between; }
.billing-info .price { font-size: 2rem; font-weight: bold; color: #2c3e50; }
.billing-info .period { color: #7f8c8d; font-size: 1.1rem; }
.savings-badge { background: #27ae60; color: white; padding: 5px 12px; border-radius: 20px; font-size: 0.9rem; font-weight: bold; }
.check-icon { color: #27ae60; font-weight: bold; margin-right: 10px; }
.billing-toggle { margin-top: 25px; border-top: 1px solid #ecf0f1; padding-top: 20px; }
.billing-toggle h3 { margin: 0 0 15px 0; color: #2c3e50; }
.toggle-group { display: flex; gap: 10px; }
.toggle-btn { flex: 1; padding: 12px 20px; border: 2px solid #e1e5e9; background: white; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; font-weight: 500; position: relative; }
.toggle-btn.active { border-color: #3498db; background: #3498db; color: white; }
.toggle-btn:hover:not(.active) { border-color: #bdc3c7; }
.save-badge { background: #e74c3c; color: white; padding: 2px 6px; border-radius: 10px; font-size: 0.8rem; margin-left: 5px; }
.payment-options { margin-bottom: 30px; }
.payment-option { border: 2px solid #e1e5e9; border-radius: 8px; padding: 20px; cursor: pointer; transition: all 0.3s ease; }
.payment-option.active { border-color: #3498db; background: rgba(52, 152, 219, 0.05); }
.option-header { display: flex; align-items: center; gap: 15px; margin-bottom: 10px; }
.option-header label { font-weight: 600; font-size: 1.1rem; color: #2c3e50; cursor: pointer; }
.payment-logo { height: 30px; margin-left: auto; }
.payment-description { color: #7f8c8d; margin: 0; margin-left: 35px; }
#paypal-button-container { margin-top: 20px; }
.loading-spinner { width: 40px; height: 40px; border: 4px solid #f3f3f3; border-top: 4px solid #3498db; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 20px; }
@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
#loading-state { text-align: center; padding: 40px; color: #7f8c8d; }
.error-message { background: #fff5f5; border: 1px solid #feb2b2; border-radius: 8px; padding: 20px; text-align: center; }
.error-message h3 { color: #e53e3e; margin: 0 0 10px 0; }
.error-message p { color: #744d4d; margin: 0 0 20px 0; }
...
.security-badges { display: flex; justify-content: center; gap: 40px; margin-top: 40px; padding: 20px; background: #f8f9fa; border-radius: 8px; }
.security-item { display: flex; align-items: center; gap: 8px; color: #2c3e50; font-weight: 500; }
.security-icon { font-size: 1.2rem; }
@media (max-width: 768px) { .checkout-content { grid-template-columns: 1fr; gap: 20px; } .plan-summary, .payment-section { padding: 20px; } .security-badges { flex-direction: column; gap: 15px; text-align: center; } .toggle-group { flex-direction: column; } }
</style>
...
function initPayPalButtons() {
	...
	paypal.Buttons({
		createOrder: function(data, actions) {
			...
			.then(function(data) {
				if (data.success) { return data.data.order_id; }
				else { throw new Error(data.data || 'Failed to create order'); }
			});
		},
