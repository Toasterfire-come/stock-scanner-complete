<?php
/**
 * Retail Trade Scanner Theme – Production Functions (Consolidated)
 */
...
add_action('widgets_init', function(){
  register_sidebar([
    'name' => __('Primary Sidebar', 'retail-trade-scanner'), 'id'=>'sidebar-1', 'description'=>__('Main sidebar area','retail-trade-scanner'),
    'before_widget'=>'<section id="%1$s" class="widget %2$s">', 'after_widget'=>'</section>', 'before_title'=>'<h2 class="widget-title">', 'after_title'=>'</h2>'
  ]);
  for ($i=1;$i<=3;$i++){
    register_sidebar([
      'name'=>sprintf(__('Footer %d', 'retail-trade-scanner'), $i), 'id'=>'footer-'.$i, 'description'=>__('Footer widget area','retail-trade-scanner'),
      'before_widget'=>'<section id="%1$s" class="widget %2$s">', 'after_widget'=>'</section>', 'before_title'=>'<h2 class="widget-title">', 'after_title'=>'</h2>'
    ]);
  }
});
...
add_action('wp_enqueue_scripts', function(){
  ...
  // Integration JS – uses plugin settings if present
  ...
  if (file_exists($js_path)){
    ...
    wp_localize_script('rts-theme-js', 'rtsConfig', [ 'apiBase' => trailingslashit($api_url), 'revenueBase' => $revenue_base ]);
  }
}, 20);

// Basic Open Graph / Twitter meta
add_action('wp_head', function(){
  $title = wp_get_document_title(); $desc = get_bloginfo('description'); $url = esc_url(home_url(add_query_arg([],'')));
...
add_action('after_switch_theme', function(){
  ...
  if (class_exists('StockScannerIntegration')){
    // Bind menu if exists
    $locations = get_nav_menu_locations(); $menu = wp_get_nav_menu_object('Stock Scanner Menu');
    if ($menu){ $id=(int)$menu->term_id; if (empty($locations['primary'])) $locations['primary']=$id; if (empty($locations['footer'])) $locations['footer']=$id; set_theme_mod('nav_menu_locations', $locations); }
  ...
  foreach ($pages as $p){
    ...
    if (!$existing){
      $pid = wp_insert_post([ 'post_title'=>$p['title'], 'post_name'=>$p['slug'], 'post_status'=>'publish', 'post_type'=>'page', 'post_content'=>'' ]);
      if ($pid && !is_wp_error($pid) && !empty($p['template']) && file_exists(get_theme_file_path($p['template']))){ update_post_meta($pid, '_wp_page_template', $p['template']); }
    } else if (!get_page_template_slug($existing->ID) && file_exists(get_theme_file_path($p['template']))){
