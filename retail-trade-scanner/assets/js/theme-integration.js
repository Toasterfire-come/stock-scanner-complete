(function(window, document){
  ...
  // Dashboard
  async function initDashboard(){ const kpis = $('#rts-kpis'); const gainers = $('#rts-trending-gainers'); const active = $('#rts-trending-active'); if(!kpis && !gainers && !active) return; setSkeleton(kpis, true); const stats = await API.get('market-stats/'); setSkeleton(kpis, false); if (kpis){ if (stats && stats.market_overview){ kpis.innerHTML = ''+
      '<div class="card p-3"><div>Total Stocks</div><div class="text-lg">'+(stats.market_overview.total_stocks||'-')+'</div></div>'+
      '<div class="card p-3"><div>Gainers</div><div class="text-lg">'+(stats.market_overview.gainers||'-')+'</div></div>'+
      '<div class="card p-3"><div>Losers</div><div class="text-lg">'+(stats.market_overview.losers||'-')+'</div></div>'+
      '<div class="card p-3"><div>Unchanged</div><div class="text-lg">'+(stats.market_overview.unchanged||'-')+'</div></div>'; } else { kpis.textContent='No market data'; } }
    const trend = await API.get('trending/'); const renderList=(el,arr)=>{ if(!el) return; if(!arr||!arr.length){ el.textContent='No data'; return; } el.innerHTML = arr.slice(0,10).map(x=>'<div class="border rounded p-2 flex justify-between"><span>'+(x.ticker||x.name)+'</span><span>'+(x.current_price||'-')+'</span></div>').join(''); };
    if (trend){ renderList(gainers, trend.top_gainers||trend.high_volume); renderList(active, trend.most_active); }
  }

  // Scanner
  function initScanner(){ const form = $('#rts-scanner-form'); const results = $('#rts-scanner-results'); const ac = $('#rts-search'); if(!form || !results) return; async function runSearch(params){ setSkeleton(results,true); const data = await API.get('stocks/', params); setSkeleton(results,false); if(!data || data.success===false){ results.textContent='No results'; return; } const rows=(data.data||data.stocks||[]).slice(0,50).map(r=>'<tr class="border-t">\n<td class="px-3 py-2">'+(r.ticker||r.symbol)+'</td>\n<td class="px-3 py-2">'+(r.company_name||r.name||'')+'</td>\n<td class="px-3 py-2">'+(r.current_price??'')+'</td>\n<td class="px-3 py-2">'+(r.change_percent??'')+'</td>\n</tr>').join(''); results.innerHTML = '<div class="overflow-x-auto rounded-lg border">\n<table class="min-w-full text-sm"><thead class="bg-muted/50 text-muted-foreground"><tr><th class="px-3 py-2 text-left">Ticker</th><th class="px-3 py-2 text-left">Name</th><th class="px-3 py-2 text-left">Price</th><th class="px-3 py-2 text-left">Change %</th></tr></thead><tbody>'+rows+'</tbody></table></div>'; }
    form.addEventListener('submit', function(e){ e.preventDefault(); const fd=new FormData(form); const params=Object.fromEntries(fd.entries()); runSearch(params); }); if (ac){ ac.addEventListener('input', debounce(async ()=>{ const v=ac.value.trim(); if(!v) return; const data = await API.get('search/', { q: v }); const sug = $('#rts-autocomplete'); if (!sug) return; sug.innerHTML = (data&&data.results?data.results.slice(0,8):[]).map(r=>'<div class="px-2 py-1 hover:underline">'+(r.ticker||r.company_name)+'</div>').join(''); }, 250)); }
  }

  // Watchlists
  function initWatchlists(){ const list=$('#rts-watchlist'); const addForm=$('#rts-watchlist-add'); async function load(){ if(!list) return; setSkeleton(list,true); const data=await API.get('watchlist/'); setSkeleton(list,false); if(!data||data.success===false){ list.textContent='No items'; return; } list.innerHTML=(data.data||[]).map(i=>'<div class="border rounded p-2 flex justify-between"><span>'+i.symbol+'</span><button data-id="'+i.id+'" class="rts-wl-del border rounded px-2">Remove</button></div>').join(''); }
    document.addEventListener('click', async (e)=>{ const btn=e.target.closest('.rts-wl-del'); if(!btn) return; e.preventDefault(); const id=btn.getAttribute('data-id'); const res=await API.del('watchlist/'+id+'/'); if(!res){ showToast('Failed to remove','error'); } load(); }); if(addForm){ addForm.addEventListener('submit', async (e)=>{ e.preventDefault(); const fd=new FormData(addForm); const body=Object.fromEntries(fd.entries()); const res=await API.post('watchlist/add/', body); showToast(res && res.success!==false ? 'Added to watchlist' : 'Failed to add', res && res.success!==false ? 'success' : 'error'); load(); }); }
    load(); }

  // Portfolio
  function initPortfolio(){ const list=$('#rts-portfolio'); const addForm=$('#rts-portfolio-add'); async function load(){ if(!list) return; setSkeleton(list,true); const data=await API.get('portfolio/'); setSkeleton(list,false); if(!data||data.success===false){ list.textContent='No holdings'; return; } const rows=(data.data||[]).map(h=>'<tr class="border-t"><td class="px-3 py-2">'+h.symbol+'</td><td class="px-3 py-2">'+h.shares+'</td><td class="px-3 py-2">'+h.avg_cost+'</td><td class="px-3 py-2">'+(h.current_price??'')+'</td><td class="px-3 py-2"><button data-id="'+h.id+'" class="rts-port-del border rounded px-2">Remove</button></td></tr>').join(''); list.innerHTML='<div class="overflow-x-auto rounded-lg border">\n<table class="min-w-full text-sm"><thead class="bg-muted/50 text-muted-foreground"><tr><th class="px-3 py-2 text-left">Symbol</th><th class="px-3 py-2 text-left">Shares</th><th class="px-3 py-2 text-left">Avg Cost</th><th class="px-3 py-2 text-left">Price</th><th></th></tr></thead><tbody>'+rows+'</tbody></table></div>'; }
    document.addEventListener('click', async (e)=>{ const btn=e.target.closest('.rts-port-del'); if(!btn) return; e.preventDefault(); const id=btn.getAttribute('data-id'); const res=await API.del('portfolio/'+id+'/'); if(!res){ showToast('Failed to remove holding','error'); } load(); }); if(addForm){ addForm.addEventListener('submit', async (e)=>{ e.preventDefault(); const fd=new FormData(addForm); const body=Object.fromEntries(fd.entries()); const res=await API.post('portfolio/add/', body); showToast(res && res.success!==false ? 'Holding added' : 'Failed to add holding', res && res.success!==false ? 'success' : 'error'); load(); }); }
    load(); }
