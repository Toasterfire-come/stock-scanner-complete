...
if (!defined('WP_DEBUG') || !WP_DEBUG) {
    ...
    if (!function_exists('rts_fatal_error_handler')) {
        ...
        if ($error && in_array($error['type'], array(E_ERROR, E_CORE_ERROR, E_COMPILE_ERROR, E_PARSE))) {
            ...
            $error_template = get_template_directory() . '/templates/error-500.php';
            if (file_exists($error_template)) {
                include $error_template;
            } else {
                echo '<h1>Website Temporarily Unavailable</h1><p>We apologize for the inconvenience. Please try again later.</p>';
            }
            exit;
        }
        }
...
add_action('wp_head', function(){ 
    $api_url = get_option('stock_scanner_api_url', ''); 
    if (!$api_url) return; 
    $p = wp_parse_url($api_url); 
    if (!$p || empty($p['scheme']) || empty($p['host'])) return; 
    $origin = esc_url($p['scheme'] . '://' . $p['host']); 
    echo "\n<link rel=\"preconnect\" href=\"{$origin}\" crossorigin>"; 
    echo "\n<link rel=\"dns-prefetch\" href=\"//{$p['host']}\">"; 
}, 3);
...
add_action('wp_head', function(){ 
    $org = [ 
        ...
        'url'=>home_url('/'), 
        'logo'=> (function_exists('has_custom_logo') && has_custom_logo()) ? wp_get_attachment_image_url(get_theme_mod('custom_logo'),'full') : '' 
    ]; 
...
add_action('wp_head', function(){ 
    ...
    $image = (is_singular() && has_post_thumbnail()) ? esc_url(get_the_post_thumbnail_url(null, 'large')) : ''; 
...
// Theme initialization - FIXED: Move this into a proper hook
add_action('after_setup_theme', function() {
...
add_action('admin_post_nopriv_rts_subscribe','rts_handle_subscribe_form'); 
add_action('admin_post_rts_subscribe','rts_handle_subscribe_form');
function rts_handle_subscribe_form(){ 
    if(!isset($_POST['rts_subscribe_nonce'])||!wp_verify_nonce($_POST['rts_subscribe_nonce'],'rts_subscribe')) rts_safe_redirect_back('subscribed','0'); 
    $email=isset($_POST['email'])?sanitize_email($_POST['email']):''; 
    if(!is_email($email)) rts_safe_redirect_back('subscribed','0'); 
    $admin=get_option('admin_email'); 
    @wp_mail($admin, __('New Subscription','retail-trade-scanner'), sprintf(__('New subscription: %s','retail-trade-scanner'), $email)); 
    rts_safe_redirect_back('subscribed','1'); 
}

add_action('admin_post_nopriv_rts_contact','rts_handle_contact_form'); 
add_action('admin_post_rts_contact','rts_handle_contact_form');
function rts_handle_contact_form(){ 
    if(!isset($_POST['rts_contact_nonce'])||!wp_verify_nonce($_POST['rts_contact_nonce'],'rts_contact')) rts_safe_redirect_back('contact','error'); 
...
add_action('wp_footer', function(){ 
    ...
    if(empty($api_url)||empty($api_secret)){ 
        echo '<div style="position:fixed;bottom:12px;right:12px;background:#111827;color:#fff;padding:10px 14px;border-radius:8px;box-shadow:0 6px 20px rgba(0,0,0,.3);z-index:9999;opacity:.9">'.esc_html__('Stock Scanner plugin needs configuration: set API URL and Secret in Settings → Stock Scanner.','retail-trade-scanner').'</div>'; 
    } 
});
...
function rts_safe_include_template($template_path, $variables = array()) {
    if (!file_exists($template_path)) {
        if (function_exists('rts_log_error')) {
