...
### 10) Manual test plan (sponsor demo script)
1. Visit `/features` and `/pricing`; confirm plan tiers, TRIAL banner, and referral banner when `?ref=ABCDE`.
2. Create account at `/auth/sign-up` using a new email; verify username auto‑derivation; accept terms; submit.
3. On redirect to Plan Selection, toggle annual billing and observe prices; select Silver.
4. At `/checkout`, confirm pricing meta loaded, enter TRIAL code, apply; verify adjusted Today/Next billing.
5. Complete PayPal checkout; verify redirect to `/checkout/success` and that Current Plan updates.
6. Navigate to `/account/billing`; confirm recent payment and invoice download.
7. Open `/account/notifications` and toggle a few settings; save and confirm success.
8. For enterprise: open `/enterprise/quote-request`, submit a sample request; confirm success steps.
9. For white‑label (Gold): open `/enterprise/white-label`, load/save a branding change; confirm status alert.

### 11) Key environment variables
- `REACT_APP_BACKEND_URL` (required) — API root base; used to build `API_ROOT` and auth/social links.
- `REACT_APP_SENTRY_DSN` — enable Sentry.
- `REACT_APP_VERSION` — release tag for Sentry.
- `REACT_APP_REFERRAL_QUERY_KEYS` — comma‑separated list of accepted referral query keys.
- PayPal (examples from code and deployment notes):
...
- Profile update endpoint is not yet supported server‑side; the client surfaces a friendly message.
- Onboarding wizard currently stores choices client‑side only; persistence TBD.
- Payment method update in Billing UI shows placeholders; connect to billing portal/provider for production.
- 2FA assumes backend endpoints exist; ensure `/auth/verify-2fa/` and `/auth/resend-2fa/` are enabled.
- SSO is presented as an enterprise option; scope and delivery via enterprise engagement.

### 13) Quick index (routes and endpoints)
