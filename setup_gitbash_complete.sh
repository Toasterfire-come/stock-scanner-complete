...
print(" PyMySQL not available, using default MySQL driver")
EOF
...
self.stdout.write(' Loading NASDAQ-Only Tickers (Windows)')
...
self.stdout.write(f' NASDAQ stocks in database: {Stock.objects.filter(exchange="NASDAQ").count()}')
self.stdout.write('')
...
print(' yfinance connection limited')
except Exception as e:
