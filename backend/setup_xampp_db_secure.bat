...
REM Update .env file for Django settings in place
set ENV_FILE=%SCRIPT_DIR%\.env
echo [STEP] Updating %ENV_FILE% with DB settings...

REM Create file if it does not exist
if not exist "%ENV_FILE%" (
  echo # Generated by setup_xampp_db_secure.bat > "%ENV_FILE%"
)

REM Helper: update or append a key=value in %ENV_FILE%
for %%V in (DB_ENGINE DB_NAME DB_USER DB_PASSWORD DB_HOST DB_PORT) do (
  rem delete existing line starting with key=
  powershell -NoProfile -Command "(Get-Content -Raw '%ENV_FILE%') -split '\r?\n' | Where-Object {$_ -notmatch '^%%V='} | Set-Content '%ENV_FILE%'"
)

REM Append our desired values
(
...
echo [OK] .env updated at %ENV_FILE%
