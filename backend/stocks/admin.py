...
from .models import (
    ...
    OptionsChain, OptionsContract, ImpliedVolatilitySurface, OptionsScreenerResult,
    OptionsAnalytics, OptionsWatchlist, OptionsWatchlistItem,
    NewsSource, NewsArticle, SentimentAnalysis, NewsFeed, NewsAlert, SentimentTimeseries,
...
# ============================================================================
# Strategy Ranking & Scoring Admin
# ============================================================================

@admin.register(TradingStrategy)
class TradingStrategyAdmin(admin.ModelAdmin):
    list_display = ['name', 'user', 'strategy_type', 'visibility', 'status', 'annual_return', 'clone_count', 'created_at']
    list_filter = ['strategy_type', 'visibility', 'status', 'is_featured', 'is_verified']
    search_fields = ['name', 'user__username', 'description']
    readonly_fields = ['created_at', 'updated_at', 'clone_count', 'view_count']
    
    fieldsets = (
        ('Strategy Info', {
            'fields': ('user', 'name', 'description', 'strategy_type')
        }),
        ('Configuration', {
        ...
        ('Performance', {
            'fields': ('annual_return', 'sharpe_ratio', 'max_drawdown', 'win_rate', 'profit_factor', 'total_trades')
        }),
        ('Community', {
            'fields': ('visibility', 'is_featured', 'is_verified', 'clone_count', 'view_count')
        }),
        ('Status', {
            'fields': ('status', 'last_traded_at', 'created_at', 'updated_at')
        }),
    )
...
class StrategyScoreAdmin(admin.ModelAdmin):
    list_display = ['strategy', 'total_score', 'performance_score', 'risk_score', 'consistency_score', 'is_sufficient_data']
    list_filter = ['is_sufficient_data', 'verification_status']
...
class StrategyRatingAdmin(admin.ModelAdmin):
    list_display = ['strategy', 'user', 'rating', 'created_at']
    list_filter = ['rating', 'created_at']
    search_fields = ['strategy__name', 'user__username', 'review']
    readonly_fields = ['created_at', 'updated_at']


@admin.register(StrategyClone)
class StrategyCloneAdmin(admin.ModelAdmin):
    list_display = ['original_strategy', 'cloned_strategy', 'user', 'cloned_at']
    list_filter = ['cloned_at']
    search_fields = ['original_strategy__name', 'user__username']
...
class StrategyLeaderboardAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        return False  # Leaderboard is generated by system
