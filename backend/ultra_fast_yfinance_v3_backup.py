...
class SmartProxyPool:
    ...
    def get_next(self) -> Optional[str]:
        ...
        while attempts < max_attempts:
            ...
            # Skip disabled, rate-limited, and proxies with recent consecutive failures
            if proxy not in self.disabled_proxies and not self.is_rate_limited(proxy):
    ...
    def record_failure(self, proxy: Optional[str]):
        ...
        if proxy and proxy in self.proxy_stats:
            ...
            # Disable if: 5+ consecutive failures OR (10+ requests AND success rate < 40%)
...
class UltraFastStockUpdater:
    ...
    def fetch_stock_data(self, symbol: str, proxy: Optional[str] = None) -> Optional[Dict]:
        ...
        """
        try:
            # Minimal delay for speed (5-10ms) - proxies handle rate limiting
