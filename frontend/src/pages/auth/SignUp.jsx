...
export default function SignUp() {
  ...
  const onSubmit = async (values) => {
    const res = await endpoints.auth.signup(values);
    if (!res.ok) {
      // Gracefully handle missing endpoint
      toast.error(res.error || 'Sign up not available. Please contact support or use admin to create account.');
      return;
    }
  ...
  return (
    <div className="max-w-md mx-auto p-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        ...
        <button disabled={isSubmitting} className="w-full px-3 py-2 rounded bg-primary text-primary-foreground">{isSubmitting ? 'Creating...' : 'Create account'}</button>
      </form>
