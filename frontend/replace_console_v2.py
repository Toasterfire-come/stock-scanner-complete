...
def main():
    ...
    print(f"LOGGER_FILE: {LOGGER_FILE}")
    ...
    for ext in ['*.js', '*.jsx']:
        for file_path in SRC_DIR.rglob(ext):
            ...
            if file_path.resolve() == LOGGER_FILE.resolve():
                print(f"Skipping logger file: {file_path}")
                continue

            # Skip node_modules and other directories
            str_path = str(file_path)
            if 'node_modules' in str_path or '__tests__' in str_path:
                continue
