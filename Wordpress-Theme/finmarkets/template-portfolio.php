...
<section class="section">
  <div class="container">
    <div class="content">
      ...
      <p class="muted">Demo-only portfolio stored locally in your browser. Add/remove holdings.</p>
    </div>
...
(function(){
  ...
  function render(){
    const body = $('#pfBody');
    const rows = getPF().map((r,i)=>`<tr><td class="mono">${r.t}</td><td>${r.q}</td><td>${r.c}</td><td><button data-i="${i}" class="btn">Remove</button></td></tr>`).join('');
    body.innerHTML = rows || '<tr><td colspan="4" class="muted">No holdings yet.</td></tr>';
  }
  document.addEventListener('DOMContentLoaded', function(){
    render();
    $('#pfAdd').addEventListener('click', ()=>{
      const t=$('#pfTicker').value.trim().toUpperCase();
      const q=parseFloat($('#pfQty').value||'0');
      const c=parseFloat($('#pfCost').value||'0');
      if(!t||q<=0||c<=0) return;
      const pf=getPF(); pf.push({t,q,c}); setPF(pf); render();
      $('#pfTicker').value=''; $('#pfQty').value=''; $('#pfCost').value='';
    });
    $('#pfBody').addEventListener('click', (e)=>{
      const b=e.target.closest('button[data-i]'); if(!b) return; const i=+b.dataset.i; const pf=getPF(); pf.splice(i,1); setPF(pf); render();
    })
  });
})();
</script>
