...
<header class="site-header">
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;height:64px;">
    <a href="<?php echo esc_url(home_url('/')); ?>" class="logo" style="display:flex;gap:10px;align-items:center;text-decoration:none;color:inherit;">
      ...
      <strong><?php echo esc_html__( 'Retail Trade Scanner', 'rts' ); ?></strong>
    </a>
    <nav class="nav" aria-label="Primary">
      <?php wp_nav_menu([ 'theme_location' => 'primary', 'container' => false, 'fallback_cb' => 'rts_menu_fallback', 'items_wrap' => '%3$s' ]); ?>
    </nav>
    <div style="display:flex;gap:10px;align-items:center;">
      <button id="rts-theme-toggle" class="btn btn-outline" type="button"><?php echo esc_html__( 'Toggle theme', 'rts' ); ?></button>
      <a class="btn btn-outline" href="<?php echo esc_url( wp_login_url() ); ?>"><?php echo esc_html__( 'Sign in', 'rts' ); ?></a>
      <a class="btn btn-primary" href="<?php echo esc_url( site_url('/auth/sign-up') ); ?>"><?php echo esc_html__( 'Get started', 'rts' ); ?></a>
    </div>
  </div>
</header>
